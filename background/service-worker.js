...
async function handleSaveLink(info, tab) {
  try {
    ...
    if (!isConfigured) {
      await NotificationsService.showError('Please configure the extension in settings first.');
      return;
    }
    ...
    await NotificationsService.showError('Failed to save link: ' + error.message);
  }
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'checkConfiguration') {
    ...
    StorageService.isConfigured()
      ...
      .catch(error => sendResponse({ isConfigured: false, error: error.message }));
